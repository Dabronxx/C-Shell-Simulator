...
void initializeCommand(struct CommandLine * command)
{
    command->argCount = 0;
...
void copyCommand(struct CommandLine * dest, struct CommandLine * src)
{
    ...
    for (int i = 0; i < src->argCount; i++)
    {
        ...
        dest->arguments[i] = malloc(strlen(src->arguments[i]));
        
        strncpy( dest->arguments[i], src->arguments[i], strlen(src->arguments[i]));
    }
}
